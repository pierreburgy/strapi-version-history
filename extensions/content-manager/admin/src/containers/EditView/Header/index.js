...
const Header = ({
  ...
  const headerActions = useMemo(() => {
    ...
    if (layout.uid?.includes("post-review")) {
      const rollback = async () => {
        const updatedPost = await request("/post-reviews/rollback", {
          method: "POST",
          body: {
            id: initialData.id,
          },
        ...
        push(
          `/plugins/content-manager/collectionType/application::post.post/${updatedPost.id}`
        );
      };
      ...
      headerActions.unshift(reviewAction);
    }
